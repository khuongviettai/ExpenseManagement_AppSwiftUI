...
struct FilteredDetailView: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false){
            VStack(spacing: 15) {
                ...
                ExpenseCard(isFilter: true).environmentObject(expenseViewModel)
                
                CustomSegmentedComtrol()
                    .padding(.top)
                
                VStack(spacing: 15) {
                    Text(expenseViewModel.converDateToString())
                        .opacity(0.7)
                ...
                ForEach(expenseViewModel.expenses.filter{
                    ...
                    expense in TransactionCardView(expense: expense)
    ...
    @ViewBuilder
    func CustomSegmentedComtrol()->some View{
        HStack(spacing: 0) {
            ForEach([ExpenseType.income, ExpenseType.expense],id: \.rawValue){
                tab in
